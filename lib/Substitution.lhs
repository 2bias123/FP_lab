...
The caesar cipher is constructed by writing the alphabet twice, once as the normal alphabet and once with the letters shifted by a number of steps or even reversed. To create this mapping a concrete implementation of the eralier defined CipherInput typeclass for the input type Int. This instance allows integers to serve as keys for the caesar cipher. The function generates a cipher map by spliting the regular alphabet at the rotation point, and then recombining the two segments into a shifted alphabet. The shifted alphabet is then ziped with the regular alphabet, foming the mapping between the plaintext letters and the cipher letters. 
